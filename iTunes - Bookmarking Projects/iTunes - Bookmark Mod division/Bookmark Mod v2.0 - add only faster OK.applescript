...
Merci cousin. Je sais pas trop quoi dire alors je vais rester simple. Mois aussi je t'aime Pierryck.
J'ai croisé ta sœur l'autre jour. Elle m'a rappeler que tu prepare encore ton concours. bon courage a toi pour ca. Passe le bonjour a ta petite. ta sœur me dis que tu passe pas mal de temps chez elle. Tu voulais pas prendre a un appart? un que tu allai choisir et faire un "chez toi" et tout. 
Moi je suis toujours a mis-temps.  toujour pas moyen de gratter plus d'heure.
Je profiter de mes matinée. Ses derniers temps je m'amuse avec quelque script sur l'ordi. Et je lis toujours autant. Je suis mes lectures ici ( https://www.goodreads.com/user/show/4606556-ankhodd). Va jeter un coup d'oeil si tu cherche un bon bouquin ;)
En attendant la prochaine, profite et amuse toi bien. bisou

https://www.goodreads.com/user_challenges/1350776
...
tell application "iTunes"
	...
	try
		...
		if main_choice is "Add" then
			...
			set no_seconds to round no_seconds mod 60 rounding as taught in school -- TDx rounding roundAsInSchool
			...
			--	ICI has to be before variable is defined i think --
			set My_Notes to the text returned of (display dialog "Add Notes or Chapter number" default answer "")
			
			--	Set my_Notes to "querry" -- Use This in Alfred
			
			set the display_name to (current_name as string) & " " & (My_Notes as string) & ¬
			...
			display notification "Bookmarking" & ¬
				display_name ¬
		...
		if main_choice is "Play From" then
			...
			repeat with n from 1 to (count of bookmark_list)
				if choice is equal to (item n of bookmark_list) then
					...
					-- 
					--ou sinon, changer la façon dont ça recup les datas par mon format de abxn
